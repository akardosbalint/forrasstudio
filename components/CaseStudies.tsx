...
const caseStudyMeta: { name: string; domain: string; href: string; pillar: Pillar }[] = [
  {
    name: "ECO Portal",
    ...
    <section id="referenciak" className="scroll-mt-20 bg-paper">
      <div className="mx-auto max-w-6xl px-5 py-20 sm:px-8 sm:py-24">
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2">
          {caseStudyMeta.map((project, index) => {
            const projectDict = dict.projects[index];
            return (
              <CaseStudyCard key={project.domain} href={project.href} index={index}>
                ...
                <p className="mt-3 leading-relaxed text-ink/70">{projectDict.description}</p>
                ...
                >
                  {dict.pillarLabels[project.pillar]}
                </span>

                <div className="mt-4 flex flex-wrap gap-2">
                  {projectDict.modules.map((module) => (
                    <span
                      key={module}
